...
class FullyAsyncRollouter(FullyAsyncRayPPOTrainer):
    ...
    async def _process_single_sample_streaming(self, rollout_sample: RolloutSample):
        ...
        # Preserve important non_tensor_batch keys before generation overwrites them
        # Agent loop output doesn't preserve these keys from the original batch
        preserved_keys = ["data_source", "reward_model", "extra_info"]
        preserved_non_tensor = {
            k: rollout_sample.full_batch.non_tensor_batch[k]
        ...
        if not is_cancel:
            ...
            # Restore preserved non_tensor_batch keys that agent loop didn't preserve
            for k, v in preserved_non_tensor.items():
