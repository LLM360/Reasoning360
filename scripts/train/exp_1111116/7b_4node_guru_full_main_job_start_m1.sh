...
WANDB_EXPERIMENT_NAME="yolorun-$(hostname)-$(date +%Y%m%d_%H%M%S)-${BASE_MODEL##*/}-4node-guru-full"
...
"${CONDA_BIN_PATH}python" -m verl.recipe.dapo.src.main_dapo \
    ...
    actor_rollout_ref.actor.ppo_max_token_len_per_gpu=$((1024*36)) \
    ...
    actor_rollout_ref.ref.log_prob_max_token_len_per_gpu=$((1024*36)) \
    ...
    actor_rollout_ref.rollout.log_prob_max_token_len_per_gpu=$((1024*36)) \
    actor_rollout_ref.rollout.gpu_memory_utilization=0.7 \
    actor_rollout_ref.rollout.log_prob_micro_batch_size=null \
    actor_rollout_ref.rollout.tensor_model_parallel_size=1 \
    actor_rollout_ref.rollout.enable_chunked_prefill=True \
    actor_rollout_ref.rollout.max_num_batched_tokens=$((1024*36)) \
