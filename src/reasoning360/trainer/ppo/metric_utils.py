...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # `data_source` is optional; metrics must never crash training if it's missing.
        ...
        metrics.update(
            {
                ...
                f"response_length/{data_source}/clip_ratio": torch.mean(
                    torch.eq(lengths_tensor, max_response_length).float()
                ).item(),
            }
        )
