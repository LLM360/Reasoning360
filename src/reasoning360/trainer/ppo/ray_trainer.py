...
from reasoning360.trainer.ppo.metric_utils import (
...
from reasoning360.trainer.ppo.reward import compute_reward, compute_reward_async
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        from reasoning360.trainer.main_ppo import create_rl_dataset, create_rl_sampler
        ...
        if collate_fn is None:
            from reasoning360.utils.dataset.rl_dataset import collate_fn as default_collate_fn
    ...
    def _validate(self, merged: bool = False):
        ...
        # NOTE: added by Reasoning360. Collect dataset information from extra_info.
        # This tracks the specific dataset name from extra_info, which may differ from data_source.
        dataset_lst = []
        ...
        for test_data in self.val_dataloader:
            ...
            # Remove all overlapping keys from test_output_gen_batch before unioning
            # since test_batch already has the original data (prompts, input_ids, attention_mask, etc.)
            output_batch_keys = set(test_output_gen_batch.batch.keys())
            input_batch_keys = set(test_batch.batch.keys())
            keys_to_remove = output_batch_keys & input_batch_keys  # intersection
            print(f"[DEBUG _validate] test_batch.batch.keys(): {list(input_batch_keys)}")
            print(f"[DEBUG _validate] test_output_gen_batch.batch.keys(): {list(output_batch_keys)}")
            print(f"[DEBUG _validate] keys_to_remove (intersection): {list(keys_to_remove)}")
            for key in keys_to_remove:
                del test_output_gen_batch.batch[key]
            print(f"[DEBUG _validate] test_output_gen_batch.batch.keys() after removal: {list(test_output_gen_batch.batch.keys())}")
            ...
            # NOTE: added by Reasoning360. Collect dataset information from extra_info.
            # The 'dataset' field provides finer-grained categorization compared to 'data_source'.
            datasets = []
        ...
        return self._val_metrics_update(data_sources, datasets, sample_uids, reward_extra_infos_dict, sample_turns, sample_scores)

    def _val_metrics_update(self, data_sources, datasets, sample_uids, reward_extra_infos_dict, sample_turns, sample_scores):
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        ...
                        with marked_timer("gen_max", timing_raw, color="purple"):
                            ...
                            # Remove overlapping keys before union (agent loop returns input_ids, attention_mask, etc.)
                            baseline_output_keys = set(gen_baseline_output.batch.keys())
                            batch_keys_set = set(batch.batch.keys())
                            baseline_overlapping = baseline_output_keys & batch_keys_set
                            for key in baseline_overlapping:
                                del gen_baseline_output.batch[key]
                    ...
                    # Remove overlapping keys from gen_batch_output before union
                    # Agent loop returns input_ids, attention_mask etc. which may conflict with batch
                    gen_output_keys = set(gen_batch_output.batch.keys())
                    batch_keys = set(batch.batch.keys())
                    overlapping_keys = gen_output_keys & batch_keys
                    if overlapping_keys:
                        print(f"[DEBUG fit] Removing overlapping keys from gen_batch_output: {list(overlapping_keys)}")
