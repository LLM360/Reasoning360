...
class RLHFDataset(Dataset):
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            # NOTE: added by Reasoning360
            # Handle both pandas DataFrame and datasets.Dataset
            if isinstance(dataframe, pd.DataFrame):
                # For pandas DataFrame, use apply with axis=1 to process each row
                print(f"Filtering pandas DataFrame with {len(dataframe)} rows...")
                from tqdm import tqdm
                tqdm.pandas(desc=f"Filtering prompts longer than {self.max_prompt_length} tokens")
                keep_mask = dataframe.progress_apply(
                    lambda row: doc2len(row.to_dict()) <= self.max_prompt_length, 
                    axis=1
                )
                dataframe = dataframe[keep_mask].reset_index(drop=True)
            else:
                # For datasets.Dataset, use the standard filter method
