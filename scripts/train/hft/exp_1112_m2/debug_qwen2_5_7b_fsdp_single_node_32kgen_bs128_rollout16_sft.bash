...
WANDB_PROJECT="HFT" # Your wandb project name
...
if [[ -n "$RESUME_CKPT_DIR_NAME" ]]; then
    ...
    WANDB_EXPERIMENT_NAME="Qwen2.5-7B-32kgen-bs128-rollout16-sft-${TIMESTAMP}-${BASE_MODEL##*/}"
fi
...
dynamic_sft_rl_advantage_threshold=1.1  # Use SFT if advantage <= this value, otherwise use RL
...
# Run training with dual output: both to terminal and log files
"${CONDA_BIN_PATH}python" -m recipe.dapo.main_dapo \
    ...
    algorithm.dynamic_sft_rl.enable=${enable_dynamic_sft_rl} \
    algorithm.dynamic_sft_rl.advantage_threshold=${dynamic_sft_rl_advantage_threshold} \
