...
from reasoning360.recipe.fully_async_policy.vllm_rollout.vllm_async_server import FullyAsyncvLLMReplica
        ...
        **kwargs,
        ...
        try:
            with rollout_trace_attr(
                ...
                # Preserve important per-sample metadata (e.g., for metric aggregation).
                # In async pipelines the final DataProto `non_tensor_batch` is reconstructed from
                # `_InternalAgentLoopOutput.extra_fields`, so we must carry `data_source` through it.
                if "data_source" in kwargs and "data_source" not in output.extra_fields:
                    output.extra_fields["data_source"] = kwargs["data_source"]
...
class FullyAsyncAgentLoopManager(AgentLoopManager):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        outputs_with_cancel = ray.get(
            [
                worker.generate_sequences_no_post.remote(chunk, None)
                for worker, chunk in zip(self.agent_loop_workers, chunkes, strict=True)
            ]
        )
        outputs = [out[0] for out in outputs_with_cancel]
