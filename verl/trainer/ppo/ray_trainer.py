...
# Added by Raesoning360: support partial rollout
...
class RayPPOTrainer(object):
    ...
    def fit(self):
        ...
        if use_partial_rollout:
            return self.fit_partial_rollout()
    ...
    # NOTE: Added by Reasoning360: support partial rollout
    def _concat_with_left_padding(self, left: DataProto, right: DataProto):
        import torch.nn.functional as F
        if left is None:
            return right
        if right is None:
            return left
        # If both exists, since the sequence length may be different, we need to pad
        ...
        for key in left.batch.keys():
            ...
            if seq_len_left < seq_len_right:
                pad_left = seq_len_right - seq_len_left
                pad = (0,) * (2 * left.batch[key].ndim - 4) + (pad_left, 0)
                left.batch[key] = F.pad(left.batch[key], pad, value=pad_value)
            else:
                pad_right = seq_len_left - seq_len_right
                pad = (0,) * (2 * right.batch[key].ndim - 4) + (pad_right, 0)
                right.batch[key] = F.pad(right.batch[key], (pad_right, 0), value=pad_value)
        new_batch = torch.cat(batch_lst, dim=0)

        non_tensor_batch = list_of_dict_to_dict_of_list(list_of_dict=[left.non_tensor_batch, right.non_tensor_batch])
        for key, val in non_tensor_batch.items():
            non_tensor_batch[key] = np.concatenate(val, axis=0)

        return DataProto(batch=new_batch, non_tensor_batch=non_tensor_batch, meta_info=left.meta_info)
    ...
    def _post_process_partial_rollout(self, partial_rollouts: DataProto,
        ...
        responses = prompts_and_response.batch["responses"]
        pad_prompt_left = (prompts == pad_token_id).sum(dim=-1, keepdim=True)
        ...
        indices = torch.where(
            ...
            j,
            torch.where(
                j < pad_prompt_left + pad_response_right,
                j - pad_prompt_left + (seq_len - pad_response_right),
    ...
    def _post_process_raw_batch(self, batch: DataProto) -> DataProto:
        # Preprocess for the new batch: add uid and is_partial metadata
        batch.non_tensor_batch["uid"] = np.array(
            [str(uuid.uuid4()) for _ in range(len(batch.batch))],
            dtype=object,
        )
        batch.batch["is_partial"] = torch.zeros((len(batch),), dtype=torch.bool)
        return batch
    ...
    def _partial_rollout_generation(self, batch: DataProto,
        ...
        if len(batch) < self.actor_rollout_wg.world_size:
            raise NotImplementedError("modifying sampling param not implemented")
    ...
    def get_train_batch(self, data_iter,
        ...
        desired_num = train_batch_size * n
        gen_iters = 0

        def generate_one_round(desired_gen_batch_size,
            ...
            for k in timing_cur:
                timing_raw[k] = timing_raw.get(k, 0) + timing_cur[k]
            for k in metrics_cur:
                metrics[k] = metrics.get(k, 0) + metrics_cur[k]
            ...
            # FIXME: for GRPO, only pop a request to replay_buffer_full
            # when all samples are done. This needs an extra replay_buffer.
            return replay_buffer_partial, replay_buffer_full

        # Have enough data to train. Move on
        ...
        batch = replay_buffer_full[:desired_num]
        replay_buffer_full = replay_buffer_full[desired_num:]
        for k in metrics:
            # NOTE: for max page usage of each replica, we still compute the mean value.
            if "page_usage" in k:
                metrics[k] /= gen_iters
        return batch, (data_iter, replay_buffer_partial, replay_buffer_full)
