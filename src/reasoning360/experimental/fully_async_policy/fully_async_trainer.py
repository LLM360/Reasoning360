...
"""
Reasoning360 customizations for FullyAsyncTrainer.
Extends verl's base implementation with custom resource pool manager and reward manager.
"""
...
# Reasoning360 custom imports
from reasoning360.experimental.fully_async_policy.ray_trainer import (
    FullyAsyncRayPPOTrainer,
)
from reasoning360.trainer.ppo.ray_trainer import ResourcePoolManager
from reasoning360.trainer.ppo.reward import load_reward_manager
...
class FullyAsyncTrainer(FullyAsyncRayPPOTrainer):
    ...
    Reasoning360 extension with custom resource pool manager and reward manager.
    """
    ...
    def __init__(
        ...
        if self.config.async_training.use_trainer_do_validate:
            from reasoning360.trainer.main_ppo import create_rl_dataset
            from reasoning360.utils.dataset.rl_dataset import collate_fn
            ...
            print(
                f"[FullyAsyncTrainer] split before val_dataset total len: {len(val_dataset)}"
            )
            ...
            print(
                f"[FullyAsyncTrainer] split after val_dataset total len: {len(val_dataset)}"
            )
            ...
            print(
                f"[FullyAsyncTrainer] create val_dataloader with batch_size: {val_batch_size}"
            )
    ...
    def set_total_train_steps(self, total_train_steps):
        ...
        self.progress_bar = tqdm(
            total=self.total_train_steps, initial=0, desc="Training Progress"
        )
    ...
    def _init_models(self):
        ...
        self.actor_rollout_wg = (
            self.actor_wg
        )  # to be compatible with the functions that not be modified
    ...
    async def _init_async_rollout_manager(self):
        ...
        if self.config.async_training.use_trainer_do_validate:
            print(
                f"[FullyAsyncTrainer] use_trainer_do_validate: {self.config.async_training.use_trainer_do_validate}"
            )
            ...
            print(
                "[FullyAsyncTrainer] Skip async rollout manager (use_trainer_do_validate=False)"
            )
    ...
    async def fit(self):
        ...
        if self.message_queue_client is None:
            raise ValueError(
                "MessageQueue client not set. Call set_message_queue_client() first."
            )
        if self.param_synchronizer is None:
            raise ValueError(
                "param_synchronizer client not set. Call set_parameter_synchronizer() first."
            )
        ...
        if (
            self.current_param_version % self.config.rollout.test_freq != 0
            or self.local_trigger_step > 1
        ):
            await self._trigger_parameter_sync_after_step(
                validate=True, global_steps=self.global_steps
            )
            ray.get(self.param_synchronizer.wait_last_valid.remote())
            self._log_validation_data()
    ...
    def _save_checkpoint(self):
        ...
        print(
            f"[FullyAsyncTrainer] local_global_step_folder: {local_global_step_folder}"
        )
        ...
        ray.get(
            self.param_synchronizer.rollouter_save_checkpoint.remote(
    ...
    def load_checkpoint(self):
        ...
        self.global_steps = (
            self.current_param_version * self.trigger_parameter_sync_step + 1
        )
        self.last_ckpt_version = self.current_param_version
        print(
    ...
    def _collect_metrics_from_samples(self, batch, metrics):
        ...
        if hasattr(batch, "meta_info") and batch.meta_info:
            ...
            stale_count = sum(
                1 for v in samples_param_versions if self.current_param_version - v >= 1
            )
    ...
    async def _trigger_parameter_sync_after_step(
        self, validate: bool = False, global_steps: int = None
    ):
        """
        Trigger parameter synchronization after training step
        ...
        if (
            ...
            and self.config.async_training.use_trainer_do_validate
        ):
            print(
                f"[FullyAsyncTrainer] validate param version: {self.current_param_version}"
            )
    ...
    def _log_validation_data(self):
        ...
        if (
            ...
            with marked_timer("timing_s/merge_val", timing_param_sync):
                new_metrics = self._merge_validation_results(
                    self.train_val_metrics, val_metrics.metrics
                )
            if new_metrics:
                ...
                self.logger.log(
                    data=val_metrics.timing_raw, step=val_metrics.param_version
                )
        else:
            if val_metrics.metrics:
                self.logger.log(
                    data=val_metrics.metrics, step=val_metrics.param_version
                )
                pprint(
                    f"[FullyAsyncTrainer] parameter version: {val_metrics.param_version} "
    ...
    async def _validate_process(self):
        if self.config.async_training.use_trainer_do_validate:
            ...
            await self.async_rollout_manager.sleep()
            print(
                f"[FullyAsyncTrainer] validate timing_raw validate: {timing_raw['trainer/validate_time']}"
            )
        else:
            self.train_val_metrics = None
