    ...
    def _process_batch_common(self, batch, metrics, timing_raw, local_trigger_step=None):
        with marked_timer("reward", timing_raw, color="yellow"):
            ...
            if self.config.reward_model.launch_reward_fn_async:
                # NOTE:
                # Do NOT pass `self.reward_fn` across processes when it may capture dynamically
                # imported functions (e.g. from `custom_reward_function`), because Ray workers
                # won't have the `custom_module` import available on unpickle.
                # Instead, let the remote task load the reward manager locally from `config`.
